...
Framework Python 0.4.2 para criar automações integradas ao Nexus através do agent.
...
O comando `nexus venv` cria o ambiente e instala apenas o SDK por padrão; use
`nexus venv --install-requirements` se quiser instalar também as bibliotecas do bot.
Depois ele mostra o comando de ativação. Se preferir já abrir uma sessão com o
ambiente configurado, use `nexus venv --shell`.
Veja o [guia completo do SDK](../../docs/sdk.md).

A versão 0.4.2 inclui modelos tipados, `nexus validate --strict`, testes locais com
